...
The workflow is defined in [`.github/workflows/newsletter_draft.yml`](.github/workflows/newsletter_draft.yml). Edit the environment variables under the [`env`](.github/workflows/newsletter_draft.yml#L47-L54) key to customize the script execution. We primarily want to change `OPENROUTER_MODEL` and `OPENROUTER_PROMPT` to further improve on the results. Note that the script automatically appends a section for `ARTICLE_LINKS` and `NEWSLETTER_LINKS` each to the specified `OPENROUTER_PROMPT`.
